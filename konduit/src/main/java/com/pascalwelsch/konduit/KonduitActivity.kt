...
abstract class KonduitActivity<P : KonduitPresenter<V>, V : KonduitView> : TiActivity<P, V>(), KonduitView {
    ...
    protected val renderer by lazy { AndroidViewRenderer(this@KonduitActivity) }

    /**
     * Create a binding for a dynamic View.
     */
    inline fun <reified W : Widget> bind(
        ...
        val bindings = `access$renderer`.bindingsFor(key)
        ...
        bindings.add(object : ViewBinding<W> {
    ...
    @PublishedApi
    internal val `access$renderer`: AndroidViewRenderer
        get() = renderer
}